...
function scriptSize(s) {
  var l = s.length;
  return bufferutils.varIntSize(l) + l;
}
...
Transaction.ZENCASH_NUM_JS_INPUTS = 2;
Transaction.ZENCASH_NUM_JS_OUTPUTS = 2;
Transaction.ZENCASH_NOTECIPHERTEXT_SIZE = 1 + 8 + 32 + 32 + 512 + 16;
Transaction.ZENCASH_G1_PREFIX_MASK = 0x02;
Transaction.ZENCASH_G2_PREFIX_MASK = 0x0a;
...
Transaction.fromBuffer = function(buffer, __noStrict) {
  ...
  for (i = 0; i < jsLen; ++i) {
    ...
    var nullifiers = [];
    for (var j = 0; j < Transaction.ZENCASH_NUM_JS_INPUTS; j++) {
      nullifiers.push(readSlice(32));
    }
    var commitments = [];
    for (j = 0; j < Transaction.ZENCASH_NUM_JS_OUTPUTS; j++) {
      commitments.push(readSlice(32));
    }
    var ephemeralKey = readSlice(32);
    var randomSeed = readSlice(32);
    var macs = [];
    for (j = 0; j < Transaction.ZENCASH_NUM_JS_INPUTS; j++) {
    ...
    var ciphertexts = [];
    for (j = 0; j < Transaction.ZENCASH_NUM_JS_OUTPUTS; j++) {
      ciphertexts.push(readSlice(Transaction.ZENCASH_NOTECIPHERTEXT_SIZE));
    }
...
Transaction.prototype.__toBuffer = function(
  ...
  function writeCompressedG1(i) {
    writeUInt8(Transaction.ZENCASH_G1_PREFIX_MASK | i.yLsb);
    writeSlice(i.x);
  }

  function writeCompressedG2(i) {
    writeUInt8(Transaction.ZENCASH_G2_PREFIX_MASK | i.yLsb);
    writeSlice(i.x);
  }
